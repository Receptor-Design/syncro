...
<?php else : ?>
	<?php if (get_field('use_custom_footer_cta_on_resource_hub', 'option')) : ?>
		<?php if (get_field('show_footer_cta_resource_hub', 'option')) : ?>
			<section class="footer-cta footer-cta-custom">
				<div class="grid-container closer-container">
					<div class="grid-x grid-padding-x align-middle">
						<?php if (get_field('footer_cta_type_resource_hub', 'option') == 'onebutton') : ?>
							<div class="cell auto">

								<h2 class="darktext"><?php echo get_field('cta_headline_resource_hub', 'option'); ?></h2>
								<div class="cta-body darktext"><?php echo get_field('cta_body_resource_hub', 'option'); ?></div>

							</div>
							<div class="cell small-12 medium-shrink button-container">
								<?php $link = get_field('cta_button_resource_hub', 'option');
								if ($link) : $link_url = $link['url'];
									$link_title = $link['title'];
									$link_target = $link['target'] ? $link['target'] : '_self';
								?>
						...
						<?php elseif (get_field('footer_cta_type_resource_hub', 'option') == 'multibutton') : ?>
							<div class="cell text-center">
							...
							<div class="cell text-center button-container">
								<?php if (have_rows('cta_buttons_resource_hub', 'option')) : ?>
									<?php while (have_rows('cta_buttons_resource_hub', 'option')) : the_row(); ?>
										<?php $link = get_sub_field('button_resource_hub', 'option');
										if ($link) : $link_url = $link['url'];
											$link_title = $link['title'];
											$link_target = $link['target'] ? $link['target'] : '_self';
										?>
